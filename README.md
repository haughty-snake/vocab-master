...
- 인터넷 연결 없이도 학습 가능
- 학습 데이터는 로컬에 자동 저장
- 정적 파일 및 데이터 파일 자동 캐싱

### 자동 업데이트
- 새 버전 출시 시 자동 감지 (1시간마다 체크)
- 토스트 알림으로 업데이트 안내
- "업데이트" 버튼 클릭 시 즉시 적용
...
- 모달 열림 → 뒤로가기로 모달 닫기
- 다른 화면 → 뒤로가기로 홈 이동
- 홈에서 뒤로가기 → 앱 유지 (PWA 모드에서 안정적 동작)

### Service Worker 기능
- **캐시 전략**: Cache-First (오프라인 우선)
- **자동 캐시 정리**: 버전 업데이트 시 이전 캐시 삭제
- **푸시 알림 지원**: 향후 알림 기능 확장 가능
- **백그라운드 동기화**: 네트워크 복구 시 자동 동기화 지원
...
- **전체 데이터 백업**: 압축 모드에 따라 `.lzstr`(압축) 또는 `.json` 파일로 백업
- **전체 데이터 복구**: `.json`과 `.lzstr` 파일 모두 지원
  - 메모리 우선 병합: 기존 데이터와 병합 후 실제 압축 크기 계산
  - 백업 확인 모달: 복구 전 현재 데이터 백업 여부 선택 가능
  - 안전한 저장: 병합 완료 후 한 번에 저장 (중간 실패 방지)
- **용량 체크**: 실제 압축 데이터 크기 기준으로 용량 확인
- **85% 한계**: 저장소 85% 초과 시 새 단어 추가/파일 가져오기 차단
- **초기화**: 학습 기록 완전 삭제
- **백업 알림**: 마지막 백업 후 일정 기간 경과 시 알림
- **버전 관리**: 시멘틱 버저닝으로 데이터 호환성 보장

### 7. 카테고리 공유
- **사용자 정의 카테고리 내보내기**: 압축 모드에 따라 `.lzstr` 또는 `.json` 파일로 저장
- **카테고리 가져오기**: `.json`과 `.lzstr` 파일 모두 지원 (일괄 처리)
...
**파일 포맷:**
- 압축 모드 ON: `.lzstr` (LZ-String UTF16 압축, 약 60% 용량 절감)
- 압축 모드 OFF: `.json` (일반 JSON 텍스트)
...
**가져오기 용량 체크:**
- `.lzstr` 파일: 압축 해제 후 JSON 크기로 용량 계산
- `.json` 파일: 파일 크기로 바로 용량 계산
- 저장소 85% 초과 시 가져오기 제한
